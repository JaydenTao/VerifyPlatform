...
class station_solution_directory(osv.Model):
    _name = 'station.solution.directory'
    _description = 'station solution directory'
    _order = 'name'
    _columns ={
               ...
               'group_ids':fields.many2many('res.groups','rail_document_director_group_rel','item_id','group_id','Groups'),
               'dctx_ids':fields.one2many('station.solution.directory.dctx','dir_id','Context fields'),
    ...
    _defaults = {
                 'company_id':lambda s,cr,uid,c: s.pool.get('res.company')._company_default_get(cr,uid,'station.solution.directory',context=c),
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        return super(station_solution_directory,self).copy(cr, uid, id, default, context=context)
    ...
    def write(self,cr,uid,ids,vals,context=None):
        ...
        return super(station_solution_directory,self).write(cr,uid,ids,vals,context=context)
    def create(self,cr,uid,vals,context=None):
        ...
        if vals.get('station_id'):
            ...
            if parent_id:
                parent = self.browse(cr, uid, vals.get('parent_id',1), context=context)

                if parent:
                    vals.update({'station_id':parent.station_id.id})
        return super(station_solution_directory,self).create(cr,uid,vals,context)
    ...
    def name_get(self,cr,uid,ids,context=None):
        ...
        return res
    def get_full_path(self,cr,uid,id,context=None):
        """
        ...
        #     assert len(id) == 1
        #     id = id[0]
        item =self.read(cr, uid, id)

        name = item.get('name')

        # def _parent(id, path):
        #     parent=self.browse(cr, uid, id,{'name'})
        #     if parent:
    ...
    def _check_duplication(self, cr, uid, vals, ids=None, op='create'):
        ...
        if op=='write':
            for directory in self.browse(cr, SUPERUSER_ID, ids):
                if not name:
                    name=directory.name
                res=self.search(cr,uid,[('id','<>',directory.id),('name','=',name)])
                if len(res):
                    return False
        if op=='create':
            res = self.search(cr, SUPERUSER_ID, [('name','=',name)])
            if len(res):
                return False
        return True
    def on_change_parent(self, cr, uid, ids, ressource_type_id):
        return {}

class station_solution_directory_dctx(osv.Model):
    ...
    """
    _name = 'station.solution.directory.dctx'
    _description = 'Directory Dynamic Context'
    _columns = {
            'dir_id':fields.many2one('station.solution.directory','Directory',required=True,ondelete='cascade'),
